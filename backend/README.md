...
Backend for **Atajo** — a Spanish-first conversational AI agent that lives on top of an account-agnostic personal-finance backend. Built for **Platanus Hack 26 (Buenos Aires)** in the *Agentic Money* track.
...
```bash
docker run -d --name atajo-pg -p 5432:5432 \
...
```bash
docker build -t atajo-backend .
docker run --env-file .env -p 8000:8000 atajo-backend
```
