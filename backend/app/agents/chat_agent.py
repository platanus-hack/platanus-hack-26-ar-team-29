...
Para crear una billetera de Ethereum, usa la herramienta `mcp__ethereum__create_ethereum_wallet`. Usa SIEMPRE `AskUserQuestion` para preguntarle al usuario en que red la quiere crear si no lo especifico (el ID de la opcion debe ser: sepolia, holesky, polygon-amoy, arbitrum-sepolia, base-sepolia, base, PERO el LABEL o TITULO debe ser legible y en mayúscula la primer letra, ej: Base, Sepolia, etc). Luego llama a la tool.
...
1. La red en la que desea importar. Usa SIEMPRE `AskUserQuestion` para preguntar esto. El ID de las opciones DEBE SER EXACTAMENTE uno de estos: sepolia, holesky, polygon-amoy, arbitrum-sepolia, base-sepolia, base. SIN EMBARGO, el `label` (lo que ve el usuario) DEBE estar capitalizado (ej: Base, Base Sepolia, Polygon Amoy) y el `header` o pregunta debe estar bien redactado (ej: "¿En qué red querés importar tu wallet?"). No uses "Recommended" en ingles.
