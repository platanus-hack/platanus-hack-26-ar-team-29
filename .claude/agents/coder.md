...
mode: primary
tools: Read, Glob, Grep, WebFetch, WebSearch, Bash, Edit, MultiEdit, Write
hooks:
  PreToolUse:
...
Consume `.claude/artifacts/02_execution_plan.md` and build the smallest working vertical slice first. Then iterate only where it improves the core demo.
...
- Prefer Rust backend when sensible. If choosing another backend, explain why in `.claude/artifacts/03_build_log.md`.
...
Maintain `.claude/artifacts/03_build_log.md` with:
